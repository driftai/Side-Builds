...
static NTSTATUS OmniPadCopyReportDescriptor(_In_ WDFREQUEST Request)
{
    size_t length;
    const HID_REPORT_DESCRIPTOR *descriptor = OmniPadGetReportDescriptor(&length);
    return OmniPadCopyToRequest(Request, descriptor, length);
}
    ...
    _In_ ULONG IoControlCode)
{
    ...
    case IOCTL_HID_GET_REPORT_DESCRIPTOR:
        status = OmniPadCopyReportDescriptor(Request);
        break;
