...
export function useLiveApi({
  ...
  const agentVolume = useUI(state => state.agentVolume);
  ...
  useEffect(() => {
    if (!audioStreamerRef.current) {
      audioContext({ id: 'audio-out' }).then((audioCtx: AudioContext) => {
        ...
        audioStreamerRef.current.setVolume(useUI.getState().agentVolume / 100);
  ...
  useEffect(() => {
    audioStreamerRef.current?.setVolume(agentVolume / 100);
  }, [agentVolume]);
  ...
  const connect = useCallback(async () => {
    ...
    try {
      const started = await client.connect(config);
      if (!started) {
        setIsConnecting(false);
      }
    } catch (error) {
