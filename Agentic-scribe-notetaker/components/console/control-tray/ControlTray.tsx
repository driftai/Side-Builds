...
function ControlTray({ children }: ControlTrayProps) {
  ...
  const {
    showAgentEdit, showUserConfig, theme, setTheme, setShowHelpModal,
    agentVolume, setAgentVolume, micMuted, setMicMuted,
  } = useUI();
  ...
  const VolumeIcon = agentVolume === 0 ? VolumeX : agentVolume < 50 ? Volume1 : Volume2;
  ...
  useEffect(() => {
    ...
    if (connected && !isConnecting && !micMuted && audioRecorder) {
  ...
  }, [connected, isConnecting, client, micMuted, audioRecorder, addPerfLog]);
  ...
  return (
    <section className="control-tray-container">
      ...
      <div className={cn('button-group')}>
        <button
          className={cn('action-button mic-button', {
            talking: isUserSpeaking && !micMuted && connected && !isConnecting,
          })}
      ...
      <div className="control-tray-right">
        <label className="agent-volume-control" title="Agent volume">
          ...
          <input
            ...
            step="1"
            value={agentVolume}
            aria-label="Agent volume"
            onChange={e => setAgentVolume(Number(e.target.value))}
          />
          <span className="agent-volume-value">{agentVolume}%</span>
        </label>
        <button
